/*global window DOMParser document Base64*/
/**
    ...
    function OdfContainer(url) {
        ...
        function parseXml(filepath, xmldata) {
            ...
            var parser = new DOMParser(),
                node = parser.parseFromString(xmldata, 'text/xml');
            return node;
        }
        /**
         ...
         * it.
         **/
        function registerCallback(callbackfunction) {
        ...
        function createElement(Type) {
            var iface = new Type(),
                original = document.createElementNS(
        ...
        function getXmlNode(filepath, callback) {
            var callbackid = null, xmldata;
            if (callback) {
                callbackid = registerCallback(function (xmldata) {
                    callback(parseXml(filepath, xmldata));
                });
            }
            xmldata = window.qtodf.load(self.url, filepath, callbackid);
            if (callback) {
                return null;
            }
