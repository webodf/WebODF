...
core.Zip = function Zip(url, entriesReadCallback) {
    ...
    function ZipEntry(url, stream) {
        ...
        function handleEntryData(data, callback) {
            var /**@const@type{!core.ByteArray}*/
                estream = new core.ByteArray(data),
                /**@const@type{!number}*/
                esig = estream.readUInt32LE(),
                /**@const@type{!number}*/
                filenamelen,
                /**@const@type{!number}*/
                eextralen;
...
*/
        ...
        this.filename = runtime.byteArrayToString(
            stream.data.subarray(stream.pos, stream.pos + namelen),
            "utf8"
        );
    ...
    function createByteArray(successCallback, errorCallback) {
        ...
        loadAllEntries(0, function (err) {
            ...
            var i, e, codoffset, codsize,
                data = new core.ByteArrayWriter("utf8"),
                offsets = [0];
            // write entries
            for (i = 0; i < entries.length; i += 1) {
